...
pub trait Embedder: Pipeline {
    ...
    fn embed(&self, texts: &[&str]) -> Box<dyn Future<Output = Result<Vec<tensor::FTensor>, Error>>>;
}
...
pub trait Classifier: Pipeline {
    ...
    fn classify(&self, texts: &[&str]) -> impl Future<Output = Result<Vec<Vec<Annotation>>, Error>>;
}
...
pub trait TokenClassifier: Pipeline {
    ...
    /// per input.
    fn classify_tokens(&self, texts: &[&str]) -> impl Future<Output = Result<Vec<Vec<Annotation>>, Error>>;
}
