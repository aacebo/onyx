...
//! [`ModelManifest`](crate::model::ModelManifest) from a
//! [`Resource`](crate::resource::Resource) into a [`Session`], and the
//! `Session` runs typed forward passes over named tensors.
//!
//! ## Async & dyn-compatibility
//!
//! [`Runtime::load`] and [`Session::infer`] are `async fn` in trait (RPITIT,
//! stable on Rust edition 2024 — no `async-trait` dependency). As a result
//! these traits are **not** `dyn`-compatible out of the box. The intended
//! primary use is static dispatch via generics (`fn run<R: Runtime>(r: &R)`).
//! A backend that needs object safety can box internally or expose its own
